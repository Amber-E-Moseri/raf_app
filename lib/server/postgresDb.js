...
function buildDirectTransaction(client) {
  return {
    ...
    async insertTransactionSplit({ householdId, transactionId, amount, categoryId, description, linkedGoalId }) {
      const wsId = workspaceIdFromHousehold(householdId);
      const now = isoNow();
      const id = uuid();
      await client.query(
        `insert into ${POSTGRES_SCHEMA}.transaction_splits
         (id, workspace_id, transaction_id, amount, category_id, description, linked_goal_id, created_at, updated_at)
         values ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
        [id, wsId, transactionId, amount, categoryId ?? null, description ?? '', linkedGoalId ?? null, now, now],
      );
      return { id, workspaceId: wsId, transactionId, amount, categoryId: categoryId ?? null, description: description ?? '', linkedGoalId: linkedGoalId ?? null, createdAt: now, updatedAt: now };
    },
    async listTransactionSplits({ householdId, transactionId = null }) {
      ...
      const result = await client.query(
        `select id, workspace_id, transaction_id, amount, category_id, description, linked_goal_id, created_at, updated_at
    ...
    async getAccountFreshnessContext({ householdId }) {
      const [batchRes, reconRes, countRes] = await Promise.all([
        client.query(
          `SELECT account_id, MAX(created_at) AS latest_import_at
        ...
        client.query(
          `SELECT DISTINCT ON (account_id)
             account_id, resolved_at AS reconciled_at, reported_as_of
           FROM ${POSTGRES_SCHEMA}.account_reconciliations
           WHERE workspace_id = $1 AND resolved_action = 'accept_reported_balance'
           ORDER BY account_id, resolved_at DESC`,
          [householdId],
        ),
        client.query(
          `SELECT COUNT(*) AS count
           FROM ${POSTGRES_SCHEMA}.imported_transactions
           WHERE workspace_id = $1 AND status = 'unreviewed'`,
          [householdId],
        ),
      ]);

      return {
        importBatches: batchRes.rows.map((r) => ({
          accountId: r.account_id,
          latestImportAt: r.latest_import_at instanceof Date
            ? r.latest_import_at.toISOString()
            : String(r.latest_import_at),
        })),
        acceptedReconciliations: reconRes.rows.map((r) => ({
          accountId: r.account_id,
          reconciledAt: r.reconciled_at instanceof Date
            ? r.reconciled_at.toISOString()
            : (r.reconciled_at ? String(r.reconciled_at) : null),
          reportedAsOf: r.reported_as_of ? String(r.reported_as_of) : null,
        })),
        unreviewedImportCount: Number(countRes.rows[0]?.count ?? 0),
      };
    },
  };
}
