...
import { buildMonthlyReviewsRepository } from '../repositories/postgres/monthlyReviewsRepository.js';
import { buildImportedTransactionsRepository } from '../repositories/postgres/importedTransactionsRepository.js';
...
function buildDirectTransaction(client) {
  return {
    ...
    async getPdfImportQuotaStatus({ householdId }) {
      ...
      const remaining = tier === 'paid' ? 999999 : 3;
      return {
        tier,
        remaining,
    ...
    async updateHouseholdPdfQuotaTier({ householdId, tier, expiresAt = null }) {
      const household = await this.getHousehold({ householdId });
      if (!household) return null;
      const updated = {
        ...household,
        pdfImportQuotaTier: tier,
        pdfImportQuotaExpiresAt: expiresAt ?? null,
        updatedAt: isoNow(),
      };
      await client.query(
        `update ${POSTGRES_SCHEMA}.households
         set pdf_import_quota_tier = $2,
             pdf_import_quota_expires_at = $3,
             updated_at = $4,
             raw_json = $5
         where id = $1 or workspace_id = $1`,
        [workspaceIdFromHousehold(householdId), tier, expiresAt ?? null, updated.updatedAt, updated],
      );
      return clone(updated);
    },
    async getEmailPreferences({ householdId }) {
      const result = await client.query(
        `select id, workspace_id, raw_json from ${POSTGRES_SCHEMA}.email_preferences
    ...
    async upsertEmailPreferences({ householdId, patch }) {
      const now = isoNow();
      const existing = await this.getEmailPreferences({ householdId });
      const row = {
        id: existing?.id ?? uuid(),
        householdId: workspaceIdFromHousehold(householdId),
        workspaceId: workspaceIdFromHousehold(householdId),
      ...
      const def = TABLES.find((entry) => entry.table === 'email_preferences');
      const data = def.toRow(row);
      await client.query(
        `insert into ${POSTGRES_SCHEMA}.email_preferences (${def.columns.join(', ')})
    ...
    async getUserByEmail({ email }) {
      const result = await client.query(
        `select id, remi_tier, raw_json from ${POSTGRES_SCHEMA}.app_users where lower(email) = lower($1) limit 1`,
        [email],
      );
      const row = result.rows[0];
      if (!row) return null;
      const json = rowFromJson(row);
      if (!json) return null;
      return { ...json, remiTier: row.remi_tier ?? json.remiTier ?? 'free' };
    },
    ...
    async deleteAccountReconciliation({ householdId, accountId, reconciliationId }) {
      await client.query(
        `delete from ${POSTGRES_SCHEMA}.account_reconciliations
         where workspace_id = $1 and account_id = $2 and id = $3`,
        [workspaceIdFromHousehold(householdId), accountId, reconciliationId],
      );
      return true;
    ...
    async getUserById({ userId }) {
      const result = await client.query(
        `select id, remi_tier, raw_json from ${POSTGRES_SCHEMA}.app_users where id = $1 limit 1`,
        [userId],
      );
      const row = result.rows[0];
      if (!row) return null;
    ...
    // --- Remi conversation repository ---

    async createRemiConversation({ householdId, userId, title }) {
      const id = uuid();
      const now = isoNow();
      const json = { id, householdId, userId, title, createdAt: now, archivedAt: null };
      await client.query(
        `INSERT INTO ${POSTGRES_SCHEMA}.remi_conversations (id, workspace_id, user_id, title, created_at, raw_json)
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [id, householdId, userId, title, now, json],
      );
      return json;
    },
    ...
    async getRemiConversation({ conversationId, householdId }) {
      const result = await client.query(
        `SELECT raw_json FROM ${POSTGRES_SCHEMA}.remi_conversations WHERE id = $1 AND workspace_id = $2 AND archived_at IS NULL LIMIT 1`,
    ...
    async listRemiConversations({ householdId, userId }) {
      const result = await client.query(
        `SELECT raw_json FROM ${POSTGRES_SCHEMA}.remi_conversations WHERE workspace_id = $1 AND user_id = $2 AND archived_at IS NULL ORDER BY created_at DESC`,
        [householdId, userId],
      );
      return result.rows.map(rowFromJson).filter(Boolean);
    },

    async listRemiMessages({ conversationId, householdId }) {
      const result = await client.query(
        `SELECT raw_json FROM ${POSTGRES_SCHEMA}.remi_messages WHERE conversation_id = $1 AND workspace_id = $2 ORDER BY created_at ASC`,
        [conversationId, householdId],
      );
      return result.rows.map(rowFromJson).filter(Boolean);
    },

    async appendRemiMessage({ conversationId, householdId, role, content, tokensUsed = 0 }) {
      const id = uuid();
      const now = isoNow();
      const json = { id, conversationId, householdId, role, content, tokensUsed, createdAt: now };
      await client.query(
        `INSERT INTO ${POSTGRES_SCHEMA}.remi_messages (id, workspace_id, conversation_id, role, content, tokens_used, created_at, raw_json)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [id, householdId, conversationId, role, content, tokensUsed, now, json],
      );
      return json;
    },
    ...
    ...buildMonthlyReviewsRepository(client, POSTGRES_SCHEMA),
    ...buildImportedTransactionsRepository(client, POSTGRES_SCHEMA),
  };
}
