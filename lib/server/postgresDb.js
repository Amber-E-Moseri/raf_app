...
const TABLES = [
  ...
  },
  {
    ...
    stateKey: 'userHouseholds',
    scope: 'global',
    columns: ['id', 'workspace_id', 'household_id', 'user_id', 'role', 'status', 'joined_at', 'invited_by', 'raw_json'],
    toRow: (r) => ({
      id: r.id,
...
async function loadState(client) {
  ...
  for (const def of TABLES) {
    const result = await client.query(`select raw_json from ${POSTGRES_SCHEMA}.${def.table} order by created_at nulls first, id`);
