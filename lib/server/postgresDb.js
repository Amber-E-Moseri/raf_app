...
const TABLES = [
  ...
  },
  {
    ...
    scope: 'workspace',
    columns: ['id', 'workspace_id', 'rule_type', 'match_value', 'auto_apply', 'created_at', 'updated_at', 'raw_json'],
    toRow: (r) => ({
      id: r.id,
      workspace_id: r.workspaceId ?? r.householdId,
...
function buildDirectTransaction(client) {
  return {
    ...
    async createWorkspaceMember({ workspaceId, userId, role = 'member', status = 'active', invitedBy = null }) {
      ...
      await client.query(
        `insert into ${POSTGRES_SCHEMA}.workspace_members (id, workspace_id, user_id, role, status, joined_at, invited_by, raw_json)
         ...
         set role = excluded.role, status = excluded.status, invited_by = excluded.invited_by, raw_json = excluded.raw_json`,
    ...
    async createUser({ email, passwordHash = null, externalAuthProvider = null, externalAuthUserId = null }) {
      ...
      const now = isoNow();
      const row = { id: uuid(), email: normalizedEmail, passwordHash, externalAuthProvider, externalAuthUserId, createdAt: now, updatedAt: now };
      await client.query(
    ...
    async createWorkspace({ ownerUserId, name, type = 'household', defaultCurrency = 'CAD', timezone = 'America/Toronto', country = 'CA' }) {
      ...
      await client.query(
        ...
        [workspaceId, workspaceId, ownerUserId, workspace.name, timezone, workspace.activeMonth, workspace.periodStartDay, workspace.savingsFloor, workspace.savingsFloorEnabled, workspace.monthlyEssentialsBaseline, 'free', null, now, now, workspace],
      );
