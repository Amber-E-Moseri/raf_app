...
export function createInMemoryDb() {
  ...
  const tx = {
    ...
    async deleteWorkspaceById({ workspaceId }) {
      ...
      state.emailPreferences = state.emailPreferences.filter((row) => row.householdId !== workspaceId && row.workspaceId !== workspaceId);
    ...
    async deleteAccountReconciliation({ householdId, accountId, reconciliationId }) {
      state.accountReconciliations = state.accountReconciliations.filter(
        (row) => !((row.householdId === householdId || row.workspaceId === householdId)
          && row.accountId === accountId
          && row.id === reconciliationId),
      );
    ...
    async listUpcomingExpenses({ householdId: targetHouseholdId, status = null }) {
      return state.upcomingExpenses
        ...
        .filter((row) => status ? row.status === status : true)
        .sort((left, right) => compareValues(left.expectedDate, right.expectedDate) || compareValues(left.id, right.id))
        .map(clone);
    },
    ...
    async insertPaymentPaceAcknowledgement({ householdId: targetHouseholdId, debtId, paymentPeriodMonth, action }) {
      const existing = state.debtPaymentPaceAcknowledgements.find(
        (row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId)
          && row.debtId === debtId
          && row.paymentPeriodMonth === paymentPeriodMonth
          && row.action === action,
      );
      if (existing) {
        existing.acknowledgementDate = isoNow();
    ...
    async getPaymentPaceAcknowledgement({ householdId: targetHouseholdId, debtId, paymentPeriodMonth }) {
      const rows = state.debtPaymentPaceAcknowledgements
        .filter((row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId)
          && row.debtId === debtId
          && row.paymentPeriodMonth === paymentPeriodMonth)
        .sort((left, right) => compareValues(right.acknowledgementDate, left.acknowledgementDate) || compareValues(right.id, left.id));
    ...
    async getPdfImportQuotaStatus({ householdId: targetHouseholdId }) {
      const household = state.households.find((row) => row.id === targetHouseholdId);
    ...
    async updateHouseholdPdfQuotaTier({ householdId: targetHouseholdId, tier, expiresAt = null }) {
      const row = state.households.find((entry) => entry.id === targetHouseholdId);
      if (!row) return null;
      Object.assign(row, {
    ...
    async getEmailPreferences({ householdId: targetHouseholdId }) {
      return clone(state.emailPreferences.find(
        (row) => row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId,
      ) ?? null);
    },
    async upsertEmailPreferences({ householdId: targetHouseholdId, patch }) {
      const existing = state.emailPreferences.find(
        (row) => row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId,
      );
      if (existing) {
        Object.assign(existing, patch, { updatedAt: isoNow() });
        return clone(existing);
      }
      const row = {
        id: uuid(),
      ...
      state.emailPreferences.push(row);
      return clone(row);
    },
    ...
    async listImportedTransactions({ householdId: targetHouseholdId, from } = {}) {
      return state.importedTransactions
        .filter((row) => row.householdId === targetHouseholdId && (from == null || row.date >= from))
        .sort((left, right) =>
