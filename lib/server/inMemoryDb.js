...
export function createInMemoryDb() {
  ...
  const tx = {
    ...
    async insertTransactionSplit(payload) {
      const row = { id: uuid(), createdAt: isoNow(), updatedAt: isoNow(), linkedGoalId: null, ...payload };
    ...
    async listTransactionSplits({ householdId: targetHouseholdId, transactionId = null }) {
      return state.transactionSplits
        ...
        .map((row) => ({ linkedGoalId: null, ...clone(row) }));
    },
    ...
    async getAccountFreshnessContext({ householdId: targetHouseholdId }) {
      // importBatches: latest createdAt per accountId for this workspace
      const batchMap = new Map();
      for (const batch of state.importBatches) {
        const isWorkspace = batch.householdId === targetHouseholdId || batch.workspaceId === targetHouseholdId;
        if (!isWorkspace || !batch.accountId) continue;
        const existing = batchMap.get(batch.accountId);
        if (!existing || (batch.createdAt ?? '') > existing) {
          batchMap.set(batch.accountId, batch.createdAt ?? null);
        }
      }
      const importBatches = Array.from(batchMap.entries()).map(([accountId, latestImportAt]) => ({
        accountId,
        latestImportAt,
      }));

      // acceptedReconciliations: latest resolvedAt per accountId
      const reconMap = new Map();
      for (const recon of state.accountReconciliations) {
        const isWorkspace = recon.householdId === targetHouseholdId || recon.workspaceId === targetHouseholdId;
        if (!isWorkspace || recon.resolvedAction !== 'accept_reported_balance') continue;
        const existing = reconMap.get(recon.accountId);
        if (!existing || (recon.resolvedAt ?? '') > (existing.reconciledAt ?? '')) {
          reconMap.set(recon.accountId, {
            accountId: recon.accountId,
            reconciledAt: recon.resolvedAt ?? null,
            reportedAsOf: recon.reportedAsOf ?? null,
          });
        }
      }
      const acceptedReconciliations = Array.from(reconMap.values());

      // unreviewedImportCount
      const unreviewedImportCount = state.importedTransactions.filter(
        (row) => (row.householdId === targetHouseholdId || row.workspaceId === targetHouseholdId)
          && (row.status ?? 'unreviewed') === 'unreviewed',
      ).length;

      return { importBatches, acceptedReconciliations, unreviewedImportCount };
    },
  };
