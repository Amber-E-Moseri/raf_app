...
import { enumerateMonths, formatCents, nextMonth, parseMoneyToCents } from '../raf/reporting.js';
...
function activeAllocationPercents(categories) {
  return [...categories]
    ...
    .map((category) => ({
      slug: category.slug,
      allocationPercent: typeof category.allocationPercent === 'string'
        ? Number(category.allocationPercent)
        : Number(category.allocationPercent ?? 0),
    }));
}
...
function projectSavingsGrowth({ projectedIncomeCents, categories }) {
  ...
  const savingsPercent = savingsCategory?.allocationPercent ?? 0;
  return Math.floor(projectedIncomeCents * savingsPercent);
}
...
function projectedEmergencyContribution(netSurplusCents, surplusSplitRules) {
  ...
  for (const rule of activeRules) {
    const splitPercent = typeof rule.splitPercent === 'string' ? Number(rule.splitPercent) : Number(rule.splitPercent ?? 0);
