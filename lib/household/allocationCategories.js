...
function formatCategoryResponse(category) {
  return {
    id: category.id,
    snapshotId: category.snapshotId ?? null,
    effectiveFrom: category.effectiveFrom ?? null,
    supersededAt: category.supersededAt ?? null,
    name: category.label,
...
function validateNormalizedItems(items, existingCategories) {
  ...
  for (const existing of existingCategories) {
    if (!incomingSlugs.has(existing.slug)) {
      if (existing.slug === 'buffer') {
        continue;
      }

      throw new HouseholdAllocationCategoriesHttpError(
        existing.isSystem === true ? 422 : 400,
        existing.isSystem === true
          ? `system allocation categories cannot be removed: ${existing.slug}`
          : `existing allocation categories must be included in the request: ${existing.slug}`,
      );
    }
  }
