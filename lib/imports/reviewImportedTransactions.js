...
async function rememberReviewChoice(tx, householdId, importedRow, reviewDecision) {
  const shouldSaveRule = reviewDecision.saveRuleMode != null || reviewDecision.rememberChoice;
  if (!shouldSaveRule || typeof tx.upsertImportReviewRule !== 'function') {
    return null;
  }
  ...
  const rule = await tx.upsertImportReviewRule({
    ...
    ruleType: reviewDecision.saveRuleMode ?? 'suggestion',
    autoApply: reviewDecision.saveRuleMode === 'reusable_rule' && reviewDecision.autoApplyRule === true,
  });
