...
export function deriveDebtSnapshot(debt, payments = []) {
  ...
  const currentBalanceCents = startingBalanceCents - totalPaidCents;
...
export function buildDebtListResponse(debts, debtPaymentsByDebtId) {
  const snapshots = debts.map((debt) => deriveDebtSnapshot(debt, debtPaymentsByDebtId.get(debt.id) ?? []));
