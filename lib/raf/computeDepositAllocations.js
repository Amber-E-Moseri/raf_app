 ...
 * Rounding remainder is always routed to the configured buffer category.
 * Older callers that do not set `isBuffer` continue to fall back to the `buffer` slug.
 */
export function computeDepositAllocations(amount, categories) {
  ...
  const bufferIndexes = activeCategories
    .map((category, index) => (isBufferCategory(category) ? index : -1))
    .filter((index) => index >= 0);

  if (bufferIndexes.length !== 1) {
    throw new Error('Active allocation categories must include exactly one buffer category.');
  }
  const [bufferIndex] = bufferIndexes;
  ...
  return allocations.map((allocation) => {
    const { allocatedCents: _allocatedCents, ...rest } = allocation;
    return rest;
  });
}
