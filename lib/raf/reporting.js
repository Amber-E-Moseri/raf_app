...
export function computeBucketBalancesSnapshot({
  ...
  for (const transaction of transactions) {
    if (!transaction.categoryId || !transactionDeltaCentsByBucketId.has(transaction.categoryId)) {
      continue;
    }
    ...
    transactionDeltaCentsByBucketId.set(
      ...
      transactionDeltaCentsByBucketId.get(transaction.categoryId) + delta,
    );
  }
...
export function computeMonthlyBucketProgressSnapshot({
  ...
  for (const transaction of transactions) {
    if (transaction.direction !== 'debit' || !transaction.categoryId || !usedByBucketId.has(transaction.categoryId)) {
      continue;
    }

    usedByBucketId.set(
      transaction.categoryId,
      usedByBucketId.get(transaction.categoryId) + Math.abs(parseMoneyToCents(transaction.amount)),
    );
  }
