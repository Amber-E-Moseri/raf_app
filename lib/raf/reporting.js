...
export function computeMonthlyReviewSnapshot({
  ...
  const splitSumsToOne = activeRules.length === 0 || Math.abs(totalSplitBps - FRACTION_SCALE) <= SUM_TOLERANCE_BPS;
  const hasEmergencyFund = activeRules.some((rule) => rule.slug === 'emergency_fund');
  const splitConfigValid = splitSumsToOne && (activeRules.length === 0 || hasEmergencyFund);
  const splitConfigError = !splitSumsToOne
    ? 'Surplus split percentages do not sum to 100%. Update your allocation preferences to fix this.'
    : (!hasEmergencyFund && activeRules.length > 0)
      ? 'Surplus split rules must include an emergency fund destination.'
      : null;
  ...
  if (netSurplusCents > 0 && activeRules.length > 0 && splitConfigValid) {
