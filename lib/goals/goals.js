...
function formatGoalProgress(goal, bucketBalance) {
  ...
  const progressPercent = targetAmountCents === 0 ? 0 : Number(((reservedAmountCents / targetAmountCents) * 100).toFixed(2));
...
export async function listGoalProgress({ db, householdId }) {
  ...
  return db.transaction(async (tx) => {
    ...
    // RAF goal progress reflects the current reserved balance inside the bucket.
