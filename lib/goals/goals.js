...
function sumGoalLinkedTransactionCents(goalId, transactions, importedTransactions = []) {
  ...
  const transactionTotal = transactions.reduce((total, transaction) => {
    ...
    // Same semantics as sumGoalLinkedContributionCents in goalContributions.js and
    // as importedTransactions below — all three must agree or the Goals page and
    // Dashboard show different numbers for the same linked transaction.
...
export async function listGoalProgress({ db, householdId }) {
  ...
  return db.transaction(async (tx) => {
    const [goals, buckets, allCategories, incomeAllocations, transactionsResult, importedTransactions] = await Promise.all([
    ...
    // Goal progress is based only on explicit transactions linked to the goal.
    ...
    return goals
      ...
      .map((goal) => {
        ...
        return formatGoalProgress(
          ...
          bucketBalance,
          sumGoalLinkedTransactionCents(goal.id, transactions, importedTransactions),
        );
      })
