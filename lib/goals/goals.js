...
export async function listGoalProgress({ db, householdId }) {
  ...
  return db.transaction(async (tx) => {
    const [goals, buckets, incomeAllocations, transactionsResult, importedTransactions] = await Promise.all([
    ...
    return goals
      ...
      .map((goal) => formatGoalProgress(
        goal,
        bucketBalancesById.get(goal.bucketId),
