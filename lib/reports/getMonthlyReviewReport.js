...
function buildGoalReservedByBucketId({ goals, allocationCategories, transactions }) {
  const categoriesById = new Map(allocationCategories.map((category) => [category.id, category]));
  const activeGoals = goals.filter((goal) => goal.active !== false);
  const bucketIdByGoalId = new Map(activeGoals.map((goal) => [goal.id, goal.bucketId ?? goal.bucket_id ?? null]));
  const reservedByBucketId = new Map();

  for (const transaction of transactions) {
    if (transaction.direction !== 'debit' || !transaction.linkedGoalId) {
      continue;
    }

    const goalBucketId = bucketIdByGoalId.get(transaction.linkedGoalId);
    if (!goalBucketId) {
      continue;
    }

    const category = categoriesById.get(goalBucketId) ?? null;
    const bucketId = category?.id ?? goalBucketId;
    reservedByBucketId.set(bucketId, (reservedByBucketId.get(bucketId) ?? 0) + Math.abs(parseMoneyToCents(transaction.amount ?? '0.00')));
  }
