...
function sumGoalLinkedTransactionCents(goalId, transactions, importedTransactions = []) {
  ...
  const transactionTotal = transactions.reduce((total, transaction) => {
    ...
    const amountCents = parseMoneyToCents(transaction.amount ?? '0.00');
    return total + (transaction.direction === 'debit' ? -amountCents : amountCents);
  }, 0);
...
function buildMonthlyGoalBucketUsage({ goals, buckets, transactions, importedTransactions = [], categoryLookupById = new Map() }) {
  ...
  const bucketIdByGoalId = new Map(activeGoals.map((goal) => [goal.id, resolveActiveBucketId(goal.bucketId)]));
...
function buildGoalProgress({ goals, bucketBalances, transactions, importedTransactions, categoryLookupById = new Map() }) {
  ...
  return goals
    ...
    .map((goal) => {
      ...
      const goalContributionCents = Math.max(sumGoalLinkedTransactionCents(goal.id, transactions, importedTransactions), 0);
...
export async function getDashboardReport({ db, householdId, from, to, year, month }) {
  ...
  return db.transaction(async (tx) => {
    ...
    const [ytdIncomeAllocations, ytdTransactionsResult] = await Promise.all([
      tx.listIncomeAllocations({ householdId, from: yearStart, to: normalizedTo }),
      tx.listTransactions({ householdId, from: yearStart, to: normalizedTo }),
    ]);
    ...
    const monthlyGoalBucketUsage = buildMonthlyGoalBucketUsage({
      goals,
      buckets: allocationCategories,
    ...
    const ytdGoalBucketUsage = buildMonthlyGoalBucketUsage({
      goals,
      buckets: allocationCategories,
    ...
    const totalToDateGoalBucketUsage = buildMonthlyGoalBucketUsage({
      goals,
      buckets: allocationCategories,
      transactions: allTransactions,
      importedTransactions,
