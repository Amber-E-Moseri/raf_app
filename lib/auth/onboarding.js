...
export async function ensureUserOnboarded({ db, user, workspaceName = null }) {
  ...
  return db.transaction(async (tx) => {
    let appUser = await tx.getUserById({ userId });
    ...
    if (workspaces.length === 0) {
      const workspace = typeof tx.createWorkspaceRecord === 'function'
        ? await tx.createWorkspaceRecord({
          ownerUserId: appUser.id,
          name: workspaceName ?? 'Personal',
          type: 'personal',
        })
        : typeof tx.createWorkspace === 'function'
      ...
      if (typeof tx.initializeWorkspaceDefaults === 'function') {
        if (typeof tx.setSecurityContext === 'function') {
          await tx.setSecurityContext({ workspaceId: workspace.id });
        }
